...
class Box2DEnv(Env):
    ...
    def _set_state(self, state):
        splitted = np.array(state).reshape((-1, 8))
        for body, body_state in zip(self.world.bodies, splitted):
            xpos, ypos, zpos, apos, xvel, yvel, zvel, avel = body_state
            body.position = (xpos, ypos, zpos)
            body.angle = apos
            body.linearVelocity = (xvel, yvel, zvel)
    ...
    def step(self, action, observation):
        ...
        observations, etc.
        """
        reward_computer = self.compute_reward(action, observation) #In UB_env
        # forward the state
    ...
    def _get_position_ids(self):
        if self._position_ids is None:
            ...
            for idx, state in enumerate(self.extra_data.states):
                if state.typ in ["xpos", "ypos", "zpos", "apos", "dist", "angle"]:
