#Same as box2d_env_ub but getting reflections is automated
...
BIG = 1e6
class Box2DEnvUBAuto(Box2DEnv, Serializable):
    ...
    def __init__(self, *args, **kwargs):
        super(Box2DEnvUBAuto, self).__init__(*args, **kwargs)
        self.setup_spaces()
    ...
    def get_raw_obs(self, action):
        ...
        if choice == 0:
            ...
            print "We are currently measuring the new plane <" + str(h_vec) + "> at " + str(chi) + ", " + str(phi)
            self.last_discrete = ind
            
            if abs(chi - self.ans[ind][0]) <= 10.0 and abs(phi - self.ans[ind][1]) <= 10.0: yesorno = 1
            else: yesorno = 0
            ...
            print "We have moved to (" + str(chi) + ", " + str(phi) + ") for the same plane (<" + str(h_vec) + ">)"
